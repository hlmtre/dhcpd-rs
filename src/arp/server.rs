...
use libc::{
  c_void, recvfrom, sendto, sockaddr, sockaddr_ll, socket, socklen_t, AF_PACKET, SOCK_RAW,
};
...
fn listen_for_arp_requests(mappings: &mut Mappings) -> io::Result<()> {
...
fn handle_packet(fd: i32, mappings: &mut Mappings, sender: sockaddr_ll, packet: &[u8]) {
  ...
  match mappings.get(&tpa) {
    Some(&mac_addr) => {
      ...
      send_reply(fd, parsed, mac_addr, sender)
    }
    _ => {
      // working here
...
fn main() {
  ...
  match listen_for_arp_requests(&mut mappings) {
    Ok(_) => return,
